...
//buildInfo.h is generated by cmake using buildInfo.h.in template
#include "buildInfo.h"
...
int main()
{
    ...
    // create local stream
    // reroute stdout to local stream
    //      https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/dup-dup2?view=vs-2019
    //route local stream to stdout's destination
    // handler for local stream
    RerouteStdout();
...
void DoThePythonThing()
{
    ...
    //need to set the stdout, stdin, stderr
    //https://docs.python.org/3/c-api/sys.html
    //int PySys_SetObject(const char *name, PyObject *v)
    //need to create a pyobject for stdout, stdin, and stderr that will output to my stuff somehow
        //can do this via a python module (https://stackoverflow.com/questions/1956407/how-to-redirect-stderr-in-python-via-python-c-api)
        //no solve for stdin yet
            //maybe this
            //https://stackoverflow.com/questions/9589092/read-input-from-redirected-stdin-with-python
            //I don't think this will solve the asynchronous requirement tho
            //need a way to have python wait (but only locally, ie still execute in other places) while input is given
            //via the game's gui
            //only in-game console would need this support, could run it as a separate python instance

            //note: if std input in ANY script is requested, it must come from the in-game console
            //python can freeze, even if python is no longer send state mutators to the game, the game will still run
            //

            //also need to consider how to "step" through python code
            //should it execute python code for a while (timed), then return to game process
            //should it step a line at a time through python code, then return to game process

            //python needs to be able to modify game state
            //some of these modifications will be time sensitive and can't be delayed (can't randomly pause a python script)
            //python must operate asynchronously from the simulation
                //but still can handle gui dev from python somehow....
