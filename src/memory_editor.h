...
struct MemoryEditor {
    ...
    bool OptShowOptions;  // = true   // display options button/context menu.
                          // when disabled, options will be locked unless you
    ...
    bool OptUpperCaseHex;    // = true   // display hexadecimal values as "FF"
                             // instead of "ff".
    int OptMidColsCount;     // = 8      // set to 0 to disable extra spacing
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr) {
        ...
        s.HexCellWidth =
            (float)(int)(s.GlyphWidth *
                         2.5f);  // "FF " we include trailing space in the width
                                 // to easily catch clicks everywhere
        s.SpacingBetweenMidCols =
            (float)(int)(s.HexCellWidth *
                         0.25f);  // Every OptMidColsCount columns we add a bit
                                  // of extra spacing
    ...
    void DrawContents(void* mem_data_void_ptr, size_t mem_size,
        ...
        // in order to prevent click from moving the window. This is used as a
        // facility since our main click detection code doesn't assign an
        // ActiveId so the click would normally be caught as a window-move.
        ...
        if (DataEditingAddr != (size_t)-1) {
            ...
            // synchronized (because currently we can't change the scrolling
                    ...
                    struct UserData {
                        ...
                        static int Callback(ImGuiInputTextCallbackData* data) {
                            ...
                            if (data->SelectionStart == 0 &&
                                ...
                                // When not editing a byte, always rewrite its
                                // content (this is a bit tricky, since
                                // InputText technically "owns" the master copy
    ...
    void DisplayPreviewData(size_t addr, const u8* mem_data, size_t mem_size,
        ...
        switch (data_type) {
            ...
            case DataType_U16: {
                ...
                if (data_format == DataFormat_Hex) {
                    ImSnprintf(out_buf, out_buf_size, "0x%04x",
                               uint16 & 0xFFFF);
                    return;
                }
                break;
            ...
            case DataType_U64: {
                ...
                if (data_format == DataFormat_Dec) {
                    ImSnprintf(out_buf, out_buf_size, "%llu",
                               (long long)uint64);
                    return;
                }
                if (data_format == DataFormat_Hex) {
