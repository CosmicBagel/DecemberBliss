...
#define LINE_TEXT_PADDING 10
    ...
    //--------------------------------------------------------------------
    if (text == NULL)
        DrawRectangle(bounds.x, bounds.y + bounds.height / 2, bounds.width, 1, color);
    else
    {
...
static int GuiMeasureTextBox(const char *text, int length, Rectangle rec, int *pos, int mode)
{
    ...
    for (i = 0; i < length; i++, k++)
    {
        ...
        if (letter != '\n')
        {
            glyphWidth = (font.chars[index].advanceX == 0) ? (int)(font.recs[index].width * scaleFactor + spacing) : (int)(font.chars[index].advanceX * scaleFactor + spacing);
...
#define VALUEBOX_MAX_CHARS 32
    ...
    if (state == GUI_STATE_PRESSED)
    {
        ...
        if (editMode && ((framesCounter / 20) % 2 == 0))
            DrawRectangle(bounds.x + GetTextWidth(textValue) / 2 + bounds.width / 2 + 2, bounds.y + 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH), 1, bounds.height - 4 * GuiGetStyle(VALUEBOX, BORDER_WIDTH), Fade(GetColor(GuiGetStyle(VALUEBOX, BORDER_COLOR_PRESSED)), guiAlpha));
    }
    else if (state == GUI_STATE_DISABLED)
    {
            ...
            while (!feof(rgsFile))
            {
                ...
                case 'f':
                {
                    ...
                    if (charmapFileName[0] != '0')
                    {
                        ...
                        font = LoadFontEx(FormatText("%s/%s", GetDirectoryPath(fileName), fontFileName), fontSize, NULL, 0);
